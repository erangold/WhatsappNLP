...
undigraph = lambda x: x.replace("\u05f0", "׳•׳•").replace("\u05f1", "׳•׳™").replace("\u05f2", "׳™׳™").replace("\ufb4f",
                                                                                                              "׳׳").replace(
    "\u200d", "")
...
def tokenize_only_words(sent):
    tag_token_tuples = tokenize(sent)
    regex_tokens = [token for tag, token in tag_token_tuples]
    return regex_tokens
